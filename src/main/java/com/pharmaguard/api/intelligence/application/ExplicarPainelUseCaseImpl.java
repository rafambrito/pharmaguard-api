...
public class ExplicarPainelUseCaseImpl implements ExplicarPainelUseCase {
    ...
    private Object selecionarPainel(ExplicarPainelCommand command, DashboardOverviewResponse overview) {
        return switch (command.tipoPainel()) {
            ...
            case DIAGNOSTICO_GERAL -> overview;
        };
    }
    ...
    private void validarNumeros(String texto, String contexto) {
        Set<Double> valoresAutorizados = extrairValoresNumericos(contexto);
        Set<String> stringsAutorizadas = extrairNumeros(contexto);

        for (String tokenNumero : extrairNumeros(texto)) {
            if (stringsAutorizadas.contains(tokenNumero)) {
                continue;
            }

            try {
                double valorTexto = Double.parseDouble(tokenNumero);

                // Permitir numeracao estrutural/listagem comum (1 a 10)
                if (valorTexto >= 1 && valorTexto <= 10 && Math.floor(valorTexto) == valorTexto) {
                    continue;
                }

                // Verificar equivalencia numerica ou arredondamento com tolerância
                boolean correspondente = false;
                for (Double autorizado : valoresAutorizados) {
                    if (Math.abs(autorizado - valorTexto) < 0.15 || Math.abs(Math.round(autorizado) - valorTexto) < 0.01) {
                        correspondente = true;
                        break;
                    }
                }
                ...
                if (!correspondente) {
                    log.debug("Numero '{}' nao encontrado no contexto autorizado", tokenNumero);
                    throw new IllegalStateException("O gerador retornou um numero ausente do contexto autorizado: " + tokenNumero);
                }
    ...
    private Set<Double> extrairValoresNumericos(String texto) {
        Set<Double> valores = new HashSet<>();
        Matcher matcher = NUMERO.matcher(texto);
        while (matcher.find()) {
            try {
                valores.add(Double.parseDouble(matcher.group().replace(',', '.')));
            } catch (NumberFormatException ignored) {
            }
        }
        return valores;
